...
//	Preferences are no longer required when created with the Service Manager (MyEcobeeInit).
	...
	input("thermostatId", "text", title: "Serial #", description:
		"The serial number of your thermostat (not needed when using MyEcobeeInit, leave it blank)")
	input("appKey", "text", title: "App Key", description:
		"The application key given by Ecobee (not needed when using MyEcobeeInit, leave it blank")
	...
	input("ecobeeType", "text", title: "ecobee Tstat Type", description:
		"Set it to registered (by default) or managementSet (not needed when using MyEcobeeInit)")
...
*/
		...
		standardTile("switchProgram", "device.programNameForUI",  canChangeIcon: false,
			...
			state "Home", label: 'Set Program Now:${name}', action: "asleep", 
				...
				nextState: "Sleep"
			state "Sleep", label: 'Set Program Now: ${name}', action: "away", 
...
private def refresh_thermostat(thermostatId, refreshValues=null) {
	...
	String currentClimateTemplate= (foundEventClimateRef)? eventClimate.name: (!foundEvent)? currentClimate.name: device.currentValue("setClimate")  // if no program's climate set, then use current program
...
def getEquipmentStatus() {
	def equipStatus = (data.thermostatList[0].equipmentStatus.size() != 0) ? 
...
//			avgHeatingSetpoint=get_stats_attribute("heatingSetpoint",startDate,endDate,'avg',true)
		...
		if (settings.trace) {    
			...
			traceEvent(settings.logFilter, "getTips>avgTemperature=$avgTemperature", settings.trace)    
